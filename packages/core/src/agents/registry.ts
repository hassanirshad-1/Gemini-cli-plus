...
export class AgentRegistry {
  ...
  async initialize(): Promise<void> {
    ...
    // Load dynamic .md agents from AgentManager
    const agentManager = this.config.getAgentManager();
    if (agentManager) {
      await agentManager.discoverAgents(this.config.storage);
      const mdAgents = agentManager.getAgents();
      for (const mdAgent of mdAgents) {
        await this.registerAgent(
          AgentManager.metadataToAgentDefinition(mdAgent),
        );
      }
    }
