...
import { SubAgentTool } from '../agents/sub-agent-tool.js';
...
export class Config {
  ...
  async initialize(): Promise<void> {
    ...
    // Discover agents
    await this.getAgentManager().discoverAgents(this.storage);
  ...
  async createToolRegistry(): Promise<ToolRegistry> {
    ...
    // Register dynamic Sub-Agents from AgentManager
    const agents = this.getAgentManager().getAgents();
    for (const agent of agents) {
      registry.registerTool(
        new SubAgentTool(agent, this, this.getMessageBus()),
      );
    }
